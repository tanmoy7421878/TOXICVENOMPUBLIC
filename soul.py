    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @TOXICVENOMNO1.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @TOXICVENOMNO1* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @TOXICVENOMNO1* 🙏"  # Contact owner message
    else:
